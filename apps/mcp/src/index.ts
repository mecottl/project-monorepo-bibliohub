...
server.registerTool(
  ...
  async (datos) => {
    try {
      ...
      const autoresTexto =
        resultado.autores_agregados.length > 0
...
server.registerTool(
  ...
  async () => {
    ...
    const texto = editoriales.map((e) => `  • ${e.nombre}`).join('\n');
    return {
      content: [
        {
          type: 'text',
...
server.registerTool(
  ...
  async () => {
    ...
    const texto = categorias.map((c) => `  • ${c.nombre}`).join('\n');
    return {
      content: [
        {
          type: 'text',
...
server.registerPrompt(
  'generar_libros_de_autor',
  {
    ...
    argsSchema: {
      ...
      stock_bajo: z.string().describe("Cuántos libros deben tener stock bajo (ej: '2' de los 5)"),
    },
  },
